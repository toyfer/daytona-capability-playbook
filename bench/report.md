...
| PDF extract | fitz 0.0027s | pdfplumber 0.102s | fitz 38x | どちらもcap内。hostのpdf skillは「作る」側で比較対象なし |
...
- **残すもの**: 初期shell/hostに無い機能か明確な勝ちがあるものだけ。`data-sql` (3.1x), `jp-encoding` (--guess), `pdf-extract` (抜く), `docs-convert` (バッチ), `ocr-scan`, `media-ffmpeg`, `keyless-open-data` (49x/147x), `jp-law-egov` (2.9x)。
- **peekコスト**: 0.03s。web_search 1回3sの1%なので当たり所では元が取れる。外れでは純増なので「当たりそうなときだけpeek」が正しい。

## 推奨INDEX表記

`use when` に勝ち筋を1語だけ足し、速度数値はcap本文に退避。INDEXはselector、詳細はcap。
